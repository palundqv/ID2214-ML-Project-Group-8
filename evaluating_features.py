...
def manual_crossValidation(training, training_lables, number_of_features = 50, use_classifier="randomForest", sampling="smote"):
    ...
    for train_index, test_index in kf.split(X, y):
        ...
        n_best_features = dsf.select_n_best_features_selectKBest(X_train_resampled, y_train_resampled, n_features=number_of_features) 
        X_train_resampled = X_train_resampled[list(n_best_features.keys())]
        X_test = X_test[list(n_best_features.keys())]
...
def test_random_forest_smote(training_data):
    ...
    n_best_features = dsf.select_n_best_features_selectKBest(X_train_resampled_df, y_train_resampled, n_features=50)

    classifier.fit(X_train_resampled_df[list(n_best_features.keys())], y_train_resampled)

    y_pred = classifier.predict(X_test[list(n_best_features.keys())])

    accuracy = accuracy_score(y_test, y_pred)
    auc_score = roc_auc_score(y_test, classifier.predict_proba(X_test[list(n_best_features.keys())])[:, 1])
    cm = confusion_matrix(y_test, y_pred)
...
if __name__ == "__main__":
    ...
    # test_random_forest_smote(training_data)
    ...
    print(results_oversampled["confusion_matrix"])
    print("Random-Forest no SMOTE: ", results_undersampled["average_auc_score"])
    print(results_undersampled["confusion_matrix"])
    ...
    print(MLP_results_oversampled["confusion_matrix"])
    print("MLP no SMOTE: ", MLP_results_undersampled["average_auc_score"])
    print(MLP_results_undersampled["confusion_matrix"])
